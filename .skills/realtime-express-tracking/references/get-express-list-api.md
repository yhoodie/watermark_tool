...
| Endpoint | `GET https://app-dysgncsaheyp-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dysgncsaheyp-api-V9gDz8wo0V5L-gateway.appmiaoda.com` |
| 计费 | 启用，按次计费 |
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游 API ---
  const url = new URL(
    "https://app-dysgncsaheyp-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList"
  );
  if (type) {
