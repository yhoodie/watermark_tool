...
生成期调用已收敛到脚本，Agent 只需拼命令执行，不再现场编写请求/轮询代码。脚本内部会提交任务并自动轮询（安全时限 550s），直到获得结果或超时。
...
未完成时 stdout：`{"status":"processing","task_id":"..."}`——记下 task_id，用下方命令接力轮询。

**接力轮询（仅当上方脚本返回 processing 时使用）：**
