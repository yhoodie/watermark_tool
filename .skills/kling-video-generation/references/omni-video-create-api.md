...
| Endpoint | `POST https://app-dysgncsaheyp-api-oLpZb03wbNBa-gateway.appmiaoda.com/v1/videos/omni-video` |
| Auth 模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dysgncsaheyp-api-oLpZb03wbNBa-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dysgncsaheyp-api-oLpZb03wbNBa-gateway.appmiaoda.com/v1/videos/omni-video",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
