...
| Endpoint | `GET https://app-dysgncsaheyp-api-AalZz7v4QEGL-gateway.appmiaoda.com/kdi` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| third_part_domain | `app-dysgncsaheyp-api-AalZz7v4QEGL-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游 API ---
  const url = new URL(
    "https://app-dysgncsaheyp-api-AalZz7v4QEGL-gateway.appmiaoda.com/kdi"
  );
