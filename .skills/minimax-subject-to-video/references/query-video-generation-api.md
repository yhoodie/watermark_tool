...
| Endpoint | `GET https://app-dysgncsaheyp-api-GYX1bq2l5vWa-gateway.appmiaoda.com/v1/query/video_generation` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | 无请求体 |
| third_part_domain | `app-dysgncsaheyp-api-GYX1bq2l5vWa-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dysgncsaheyp-api-GYX1bq2l5vWa-gateway.appmiaoda.com/v1/query/video_generation?task_id=${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
