...
| Endpoint | `GET https://app-dysgncsaheyp-api-VaOw5V2Pbqoa-gateway.appmiaoda.com/v1/files/retrieve` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | 无请求体 |
| third_part_domain | `app-dysgncsaheyp-api-VaOw5V2Pbqoa-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dysgncsaheyp-api-VaOw5V2Pbqoa-gateway.appmiaoda.com/v1/files/retrieve?file_id=${encodeURIComponent(fileId)}`,
    {
      method: "GET",
      headers: {
