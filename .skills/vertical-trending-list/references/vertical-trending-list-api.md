...
| Endpoint | `POST https://api-ra5EZDjVyjVa@qianfan.baidubce.com/v2/tools/trending_lists/vertical` |
| Auth 模式 | `platform_managed`（traefik: true） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| Third-party Domain | `qianfan.baidubce.com` |
...
async function queryVerticalTrendingList(
  ...
  const response = await fetch(
    "https://api-ra5EZDjVyjVa@qianfan.baidubce.com/v2/tools/trending_lists/vertical",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
